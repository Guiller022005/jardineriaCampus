...
def postProducto():
    ...
    peticion = requests.post("http://172.16.103.28:50004/pedido",headers=headers, data=json.dumps(Pedido, indent=4).encode("UTF-8"))
    res = peticion.json()
...
def deletePedido(id):
    ...
    if(len(data)):
        peticion = requests.get(f"http://172.16.103.28:50004/pedido/{id}")
...
def getAllDataPedido():
    ...
    peticion = requests.get("http://172.16.103.28:50004/pedido")
    data = peticion.json()
    return data 
...
def postPedido():
    ...
    headers = {'Content-Type': 'application/json', 'charset': 'utf-8'}
    peticion = requests.post("http://172.16.103.28:50004", headers=headers, data=json.dumps(pedido))
    res = peticion.json()
