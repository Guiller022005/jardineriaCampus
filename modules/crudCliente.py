...
def deleteCliente(id):
    ...
    if(len(data)):
        peticion = requests.delete(f"http://172.16.103.28:50001/clientes/{id}")
...
def postCliente():
    ...
    peticion = requests.post("http://172.16.103.28:50001",headers=headers, data=json.dumps(cliente, indent=4).encode("UTF-8"))
    res = peticion.json()
...
def postClientes():
    ...
    headers = {'Content-Type': 'application/json', 'charset': 'utf-8'}
    peticion = requests.post("http://172.16.103.28:50001/clientes", headers=headers, data=json.dumps(cliente))
    res = peticion.json()
